...
class TestSelectEntrySlotResolution:
    ...
    async def test_select_entry_uses_active_slot(self, test_server):
        ...
        await server.call_tool_direct("memcord_name", {"slot_name": "select-test-slot"})
        await server.call_tool_direct("memcord_save", {"chat_text": "Entry content"})

        # Select entry without slot_name
        result = await server.call_tool_direct("memcord_select_entry", {"relative_time": "latest"})

        assert "Entry content" in result[0].text or "select-test-slot" in result[0].text
