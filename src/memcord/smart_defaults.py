...
class PreferenceLearningEngine:
    ...
    async def record_command(
        self, tool_name: str, parameters: dict[str, Any], context: dict[str, Any], success: bool = True
    ):
    ...
    async def get_auto_completions(
        self, tool_name: str, parameter: str, partial_value: str, context: dict[str, Any]
    ) -> list[str]:
        ...
        for pattern in patterns:
            ...
            if key in self.preferences:
                ...
                self.preferences[key] = UserPreference(
                    key=key, value=True, confidence=0.4, usage_count=1, last_used=datetime.now(), context=context
                )
    ...
    def _learn_tagging_patterns(self, tags: list[str], context: dict[str, Any]):
        ...
        for tag in tags:
            ...
            if key in self.preferences:
                ...
                self.preferences[key] = UserPreference(
                    key=key, value=tag, confidence=0.3, usage_count=1, last_used=datetime.now(), context=context
                )
    ...
    def _learn_compression_preferences(self, ratio: float, context: dict[str, Any]):
        ...
        if key in self.preferences:
            ...
            self.preferences[key] = UserPreference(
                key=key, value=ratio, confidence=0.5, usage_count=1, last_used=datetime.now(), context=context
            )
    ...
    def _get_workflow_suggestions(self, current_tool: str, context: dict[str, Any]) -> list[SmartSuggestion]:
        ...
        if current_tool in workflow_patterns:
            for next_tool, reason in workflow_patterns[current_tool]:
                ...
                suggestions.append(
                    SmartSuggestion(
                        tool_name=next_tool, parameters={}, reason=reason, confidence=confidence, category="workflow"
                    )
                )
    ...
    def _get_slot_name_completions(self, partial_value: str, context: dict[str, Any]) -> list[str]:
        ...
        if partial_value:
            ...
            if partial_value.startswith("proj"):
                completions.extend(
                    [
                        f"proj_{datetime.now().strftime('%Y_%m_%d')}",
                ...
                completions.extend(
                    [
                        f"meet_{datetime.now().strftime('%Y_%m_%d')}",
                ...
                completions.extend(
                    [
                        f"learn_{partial_value[6:]}_basics",
    ...
    def _get_tag_completions(self, partial_value: str, context: dict[str, Any]) -> list[str]:
        ...
        common_tags = [
            pref.value
            for key, pref in self.preferences.items()
            if key.startswith("common_tag.") and str(pref.value).startswith(partial_value)
        ]
    ...
    def _get_group_completions(self, partial_value: str, context: dict[str, Any]) -> list[str]:
        ...
        # Recent group paths
        recent_groups = [
            cmd["parameters"].get("group_path", "") for cmd in self.command_history if "group_path" in cmd["parameters"]
        ]
    ...
    async def _load_preferences(self):
        ...
        try:
            if self.preferences_file.exists():
                async with aiofiles.open(self.preferences_file) as f:
    ...
    async def _load_workflow_patterns(self):
        ...
        try:
            if self.patterns_file.exists():
                async with aiofiles.open(self.patterns_file) as f:
                    content = await f.read()
...
class WorkflowAutomation:
        ...
        return {
            ...
            "troubleshooting_session": {
                ...
                "steps": [
                    ...
                    {
                        "tool": "memcord_tag",
                        "params": {"action": "add", "tags": ["debugging", "troubleshooting", "{category}"]},
                    },
    ...
    async def execute_template(
        self, template_name: str, params: dict[str, str], context: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for step in template["steps"]:
            try:
                ...
                results.append({"tool": step["tool"], "params": step_params, "success": True, "result": result})

                # Record for learning
                await self.preference_engine.record_command(step["tool"], step_params, context, success=True)

            except Exception as e:
                results.append(
                    {"tool": step["tool"], "params": step.get("params", {}), "success": False, "error": str(e)}
                )
    ...
    async def batch_operation(self, operations: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
        ...
        for op in operations:
            try:
                ...
                results.append({"tool": tool_name, "params": merged_params, "success": True, "result": result})

                # Record for learning
                await self.preference_engine.record_command(tool_name, merged_params, context, success=True)

            except Exception as e:
                results.append({"tool": op["tool"], "params": op.get("params", {}), "success": False, "error": str(e)})
    ...
    def _substitute_params(self, template_params: dict[str, Any], filled_params: dict[str, str]) -> dict[str, Any]:
        ...
        for key, value in template_params.items():
            if isinstance(value, str):
                ...
                result[key] = [
                    self._substitute_string(item, filled_params) if isinstance(item, str) else item for item in value
                ]
            else:
                result[key] = value
    ...
    async def _execute_tool(self, tool_name: str, params: dict[str, Any], context: dict[str, Any]) -> Any:
