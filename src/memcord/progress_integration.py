...
from .progress_tracker import (
    ProgressTracker, OperationType, OperationResult, 
    ConsoleProgressCallback, ProgressCallback
)
from .feedback_messages import FeedbackMessageGenerator, ConfirmationManager
...
class MemcordProgressIntegration:
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def wrapper(*args, **kwargs):
                ...
                if confirmation_type:
                    confirmation_text = self.feedback_generator.create_confirmation_dialog(
                        confirmation_type, context
                    )
                    ...
                    try:
                        ...
                        if isinstance(result, dict):
                            ...
                            self.progress_tracker.complete_operation(
                                progress_context.operation_id, enhanced_result
                            )
    ...
    def _extract_context(self, func: Callable, args: tuple, kwargs: dict) -> Dict[str, Any]:
        ...
        for param_name, value in bound_args.arguments.items():
            if param_name in ['slot_name', 'slot_names', 'query', 'file_path', 
                             'source_slots', 'target_slot', 'compression_ratio',
                             'content']:
    ...
    def _get_confirmation_type(self, operation_type: OperationType, 
...
def track_save_operation(integration: MemcordProgressIntegration):
    ...
    return integration.track_operation(
        OperationType.SAVE,
        "Saving content to memory slot"
    )
...
async def track_long_operation(storage_dir: Path, operation_type: OperationType,
                              description: str, operation_func: Callable,
                              callback: Optional[ProgressCallback] = None) -> Any:
...
def create_progress_callback(show_console: bool = True, 
                           show_details: bool = True) -> ProgressCallback:
...
class ProgressContext:
    ...
    def __init__(self, storage_dir: Path, operation_type: OperationType,
    ...
    def __enter__(self):
        self.progress_context = self.tracker.track_operation(
            self.operation_type, self.description, 
            self.total_steps, self.callback
        ).__enter__()
...
_global_integration: Optional[MemcordProgressIntegration] = None
...
def get_progress_integration() -> Optional[MemcordProgressIntegration]:
