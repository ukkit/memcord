    ...
    def get_user_message(self) -> str:
        ...
        if self.context:
            relevant_context = {k: v for k, v in self.context.items() 
                              if k in ['operation', 'slot_name', 'file_path', 'query']}
            if relevant_context:
...
class ErrorHandler:
    ...
    def _convert_exception(self, error: Exception, operation: str = None, context: Dict[str, Any] = None) -> MemcordError:
        ...
        if isinstance(error, ValueError):
            return ValidationError(
                str(error),
                context=context,
                recovery_suggestions=["Check your input values and try again"]
            )
    ...
    def create_rate_limit_error(self, operation: str, limit: int) -> RateLimitError:
        ...
        return RateLimitError(
            f"Rate limit exceeded for {operation} ({limit} requests/minute)",
            operation=operation,
            limit=limit
        )
