...
from .models import MemorySlot, MemoryEntry, SearchResult, SearchQuery
...
class IncrementalSearchIndex:
    ...
    async def _load_incremental_changes(self):
        ...
        for changes_file in changes_files:
            try:
                async with aiofiles.open(changes_file, 'r', encoding='utf-8') as f:
                    data = await f.read()
...
class DeltaCompressor:
    ...
    async def create_delta(self, slot_name: str, old_slot: MemorySlot, new_slot: MemorySlot) -> Path:
        ...
        if old_slot.tags != new_slot.tags:
            delta_data['operations'].append({
                'type': 'update_tags',
                'old_tags': list(old_slot.tags),
                'new_tags': list(new_slot.tags)
            })
            
        if old_slot.group_path != new_slot.group_path:
            delta_data['operations'].append({
                'type': 'update_group',
                'old_group': old_slot.group_path,
                'new_group': new_slot.group_path
            })
        ...
        for operation in delta_data['operations']:
            ...
            if op_type == 'add_entry':
                ...
                updated_slot.entries = [
                    entry for entry in updated_slot.entries 
                    if entry.timestamp.isoformat() != timestamp_str
                ]
...
class StorageDefragmenter:
    ...
    async def analyze_fragmentation(self) -> Dict[str, float]:
        ...
        try:
            ...
            if slot_files:
                slot_names = sorted([f.stem for f in slot_files])
                expected_count = len(slot_names)
    ...
    async def defragment_storage(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if not dry_run:
                ...
                improvement = initial_fragmentation['overall_fragmentation'] - final_fragmentation['overall_fragmentation']
    ...
    async def _consolidate_index_files(self):
        ...
        async with aiofiles.open(main_index_path, 'r', encoding='utf-8') as f:
            main_data = json.loads(await f.read())
        
        # Apply all changes to main index
        for change_file in change_files:
            try:
                async with aiofiles.open(change_file, 'r', encoding='utf-8') as f:
...
class QuotaManager:
    ...
    async def check_quota_compliance(self) -> Tuple[bool, List[str]]:
        ...
        if stats.total_size_mb > self.config.max_total_size_mb:
            violations.append(
                f"Total storage size ({stats.total_size_mb:.1f} MB) exceeds limit ({self.config.max_total_size_mb:.1f} MB)"
            )
        
        # Check slot count
        if stats.total_slots > self.config.max_slots:
            violations.append(
                f"Number of slots ({stats.total_slots}) exceeds limit ({self.config.max_slots})"
            )
    ...
    async def enforce_quota(self, slot_name: str, new_slot_size_mb: float) -> Tuple[bool, str]:
        ...
        if projected_total_size > self.config.max_total_size_mb:
            return False, f"Operation would exceed total size limit ({projected_total_size:.1f}/{self.config.max_total_size_mb:.1f} MB)"
    ...
    async def suggest_cleanup_actions(self) -> List[str]:
        ...
        if candidates:
            suggestions.append(f"Archive {len(candidates)} old slots (not accessed in {self.config.cleanup_threshold_days} days)")
...
class AlertManager:
    ...
    def __init__(self, storage_dir: Path, quota_config: Optional[QuotaConfig] = None):
        self.storage_dir = Path(storage_dir)
    ...
    async def run_optimization_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.quota_config and self.quota_config.auto_cleanup_enabled:
                monitor = StorageMonitor(self.storage_dir)
                cleanup_candidates = await monitor.identify_cleanup_candidates(
                    self.quota_config.cleanup_threshold_days
                )
