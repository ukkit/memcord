...
from .progress_tracker import OperationType, OperationResult
...
class SuggestionEngine:
    ...
    def __init__(self):
        self._suggestion_rules: Dict[str, List[Callable]] = {
            ...
            OperationType.SEARCH.value: [
                self._suggest_search_refinements,
                self._suggest_result_actions
            ],
            OperationType.MERGE.value: [
                self._suggest_post_merge_actions,
                self._suggest_cleanup
            ],
            OperationType.IMPORT.value: [
                self._suggest_import_organization,
                self._suggest_validation
            ],
            OperationType.COMPRESS.value: [
                self._suggest_compression_follow_up
            ],
            OperationType.ARCHIVE.value: [
                self._suggest_archive_maintenance
            ]
        }
    ...
    def generate_suggestions(self, 
                           ...
                           context: Dict[str, Any]) -> List[NextStepSuggestion]:
        ...
        if slot_name:
            suggestions.extend([
                ...
                NextStepSuggestion(
                    ...
                    command=f"memcord_search --query 'similar topics'",
                    priority=3,
                    category="discovery"
                )
            ])
    ...
    def _suggest_import_organization(self, result: OperationResult, context: Dict[str, Any]) -> List[NextStepSuggestion]:
    ...
    def _suggest_compression_follow_up(self, result: OperationResult, context: Dict[str, Any]) -> List[NextStepSuggestion]:
    ...
    def _suggest_archive_maintenance(self, result: OperationResult, context: Dict[str, Any]) -> List[NextStepSuggestion]:
        """Suggest maintenance after archive operations."""
        suggestions = []

        archived_count = (context.get('archived_count', 0) or
                         result.details.get('slots_deleted', 0) or
                         result.details.get('archived_count', 0) or
                         result.details.get('slots_processed', 0))
    ...
    def _generate_general_suggestions(self, result: OperationResult, context: Dict[str, Any]) -> List[NextStepSuggestion]:
...
class ConfirmationManager:
    ...
    def create_confirmation(self, operation_type: str, context: Dict[str, Any]) -> Optional[ConfirmationDialog]:
...
class FeedbackMessageGenerator:
    ...
    def create_confirmation_dialog(self, operation_type: str, context: Dict[str, Any]) -> Optional[str]:
    ...
    def _calculate_impact(self, operation_type: OperationType, 
                         result_data: Dict[str, Any], context: Dict[str, Any]) -> ImpactSummary:
    ...
    def _create_detailed_message(self, operation_type: OperationType,
                                result_data: Dict[str, Any], impact: ImpactSummary) -> str:
    ...
    def _has_undo_capability(self, operation_type: OperationType, 
        ...
        return (operation_type in undoable_operations and 
                'undo_info' in result_data)