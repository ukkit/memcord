...
from typing import Dict, List, Optional, Any, Set, Tuple, Union, Callable, Deque
...
class MemoryOptimizer:
    ...
    def add_weak_reference(self, obj, callback: Optional[Callable] = None) -> Optional[weakref.ReferenceType]:
...
class MemoryManager:
    ...
    def __init__(self, enable_tracemalloc: bool = True, memory_limit_mb: Optional[float] = None,
    ...
    async def collect_stats(self) -> MemoryStats:
        ...
        stats = MemoryStats(
            ...
            peak_memory_mb=max(memory_info.rss / 1024 / 1024, 
                              self.stats_history[-1].peak_memory_mb if self.stats_history else 0),
    ...
    async def check_memory_limits(self):
        ...
        if current_memory >= self.memory_limit_mb * self.critical_threshold:
            alert = MemoryAlert(
                alert_type="critical",
                message=f"Memory usage critical: {current_memory:.1f}MB >= {self.memory_limit_mb * self.critical_threshold:.1f}MB",
            ...
            alert = MemoryAlert(
                alert_type="warning",
                message=f"Memory usage warning: {current_memory:.1f}MB >= {self.memory_limit_mb * self.warning_threshold:.1f}MB",
                current_memory_mb=current_memory,
    ...
    async def optimize_memory(self):
        ...
        cleaned_refs = self.optimizer.cleanup_weak_references()
    ...
    def get_memory_trend(self, minutes: int = 30) -> Dict[str, float]:
        ...
        return {
            ...
            "memory_change_percent": (current_memory - initial_memory) / initial_memory * 100 if initial_memory > 0 else 0,
    ...
    def configure_limits(self, memory_limit_mb: float, warning_threshold: float = 0.8, 
