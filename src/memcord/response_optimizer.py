...
class ResponseOptimizer:
    ...
    def _format_compact(self, content: str) -> List[TextContent]:
        ...
        for line in lines:
            if line.startswith('•') or line.startswith('-') or line.startswith('*'):
                ...
                if compact_lines and (compact_lines[-1].startswith(marker) or
                                    compact_lines[-1].startswith('•') or compact_lines[-1].startswith('-')):
        ...
        for line in lines[:30]:  # First 30 lines only
            ...
            if (stripped and
                (stripped.startswith(('=', '#', '**', '•', '-', '*')) or
                 len(stripped) > 50)):  # Headers or substantial content
    ...
    def optimize_list_response(self, items: List[Dict[str, Any]], 
                             max_items: int = 10) -> List[TextContent]:
    ...
    def optimize_search_results(self, results: List[Dict[str, Any]], 
                               query: str) -> List[TextContent]:
...
def format_timestamp_compact(timestamp: Union[str, datetime]) -> str:
